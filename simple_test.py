...
def unprocess(img, scale, std=0.5, mean=0.5):
    ...
    temp = np.zeros((4,312,416),'uint16')
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        ImFakeB = unprocess(fakeB.data[0], scale=2**14)
        ImRecA = unprocess(recA.data[0], scale=2**12)
        ImRecB = unprocess(recB.data[0], scale=2**14)
        ImFakeA = unprocess(fakeA.data[0], scale=2**12)
        
        ImRealA = unprocess(data["A"].data[0], scale=2**12)
        ImRealB = unprocess(data["B"].data[0], scale=2**14)

        # print(data)
        img_ind = data["A_paths"][0].split("\\")[-1].split("_")[:-1]

        # ImFakeB.save(rtname+"\\FakeB\\"+"_".join(img_ind)+"_FakeB.tiff")
        imwrite(rtname+"\\FakeB\\"+"_".join(img_ind)+"_FakeB.tiff",ImFakeB)

        # ImFakeA.save(rtname+"\\FakeA\\"+"_".join(img_ind)+"_FakeA.tiff")
        imwrite(rtname+"\\FakeA\\"+"_".join(img_ind)+"_FakeA.tiff",ImFakeA)

        # ImRecB.save(rtname+"\\RecB\\"+"_".join(img_ind)+"_RecB.tiff")
        imwrite(rtname+"\\RecB\\"+"_".join(img_ind)+"_RecB.tiff",ImRecB)

        # ImRecA.save(rtname+"\\RecA\\"+"_".join(img_ind)+"_RecA.tiff")
        imwrite(rtname+"\\RecA\\"+"_".join(img_ind)+"_RecA.tiff",ImRecA)

        # ImRealA.save(rtname+"\\RealA\\"+"_".join(img_ind)+"_RealA.tiff")
        imwrite(rtname+"\\RealA\\"+"_".join(img_ind)+"_RealA.tiff",ImRealA)

        # ImRealB.save(rtname+"\\RealB\\"+"_".join(img_ind)+"_RealB.tiff")
        imwrite(rtname+"\\RealB\\"+"_".join(img_ind)+"_RealB.tiff",ImRealB)
