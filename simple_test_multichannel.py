...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        ImFakeB = unprocess(fakeB.data[0], scale=2**12)
        ImRecA = unprocess(recA.data[0], scale=3, mean=0.7)
        ImRecB = unprocess(recB.data[0], scale=2**12)
        ImFakeA = unprocess(fakeA.data[0], scale=3, mean=0.7)
        
        ImRealA = unprocess(data["A"].data[0], scale=3, mean=0.5)
        ImRealB = unprocess(data["B"].data[0], scale=2**12)
