...
class ADCnumpyDataset(BaseDataset):
    """
...
class ACast(object):
    ...
    def __call__(self, pic):
        ...
        # print(pic.max())
        img = np.maximum(np.array(pic, np.float32), -1)
        img = np.minimum(np.array(img, np.float32), 1)
        # print(img.max())
        #img = img/(5000)
        # img = np.squeeze(img)
        ...
        img1 = np.zeros((3,256,256), dtype=float)-0.9
        img1[0,64:192,64:192] = img[0,:,:]
        img1[1,64:192,64:192] = img[1,:,:]
        img1[2,64:192,64:192] = img[2,:,:]
        img1 = from_numpy(img1.copy())
        # print(img.shape)
        return img1.float()
...
class BCast(object):
    ...
    def __call__(self, pic):
        ...
        # return img.float()
        # print(pic.min())
        img = np.minimum(np.array(pic, np.float32), 1)
        img = np.maximum(np.array(pic, np.float32), -1)
        # print(img.max())
        #img = img/(3000)
        # img = np.squeeze(img)
