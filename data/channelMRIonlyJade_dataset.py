...
class ChannelMRIonlyJadeDataset(BaseDataset):
    """
...
class ACast(object):
    ...
    def __call__(self, pic):
        ...
        img = np.minimum(np.array(pic, np.int16), 2)
        ...
        # print(img.shape)
        # x = random.randint(0, np.maximum(0, 256 - 128))
        # y = random.randint(0, np.maximum(0, 256 - 128))
        # print(x)
...
class BCast(object):
    ...
    def __call__(self, pic):
        img = np.minimum(np.array(pic, np.int16), 2**13)
        # img = np.maximum(np.array(pic, np.int16), 0)
        # img = img - 64
        if not self.no_flip and random.random() < 0.5:
           img = np.flip(img,2)
